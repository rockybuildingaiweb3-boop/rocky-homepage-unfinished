// scripts/build-80-skills-v2.mjs
import fs from 'fs';

export const SKILL_ROWS_8 = [
  { row: 1, title: '核心前端与交互动效', subtitle: 'Core Frontend & Kinetic Motion' },
  { row: 2, title: '3D 空间计算与图形资产', subtitle: '3D Spatial Computing & Graphics' },
  { row: 3, title: '服务端、数据与基础设施', subtitle: 'Backend, Data & Infrastructure' },
  { row: 4, title: 'Web3 与去中心化架构', subtitle: 'Web3 & Decentralized Architecture' },
  { row: 5, title: 'AI 模型与调用工程', subtitle: 'AI Models & Inference Engineering' },
  { row: 6, title: 'AI Agent 与工作流编排', subtitle: 'AI Agents & Workflow Orchestration' },
  { row: 7, title: 'RAG 知识库与检索工程', subtitle: 'RAG Knowledge Base & Retrieval' },
  { row: 8, title: '工程化部署、测试与稳定性', subtitle: 'Engineering Deployment, Testing & Reliability' },
];

export const SKILLS_80_V2 = [
  // ─── ROW 1: 核心前端与交互动效 (10) ───
  {
    id: 'typescript',
    name: 'TypeScript',
...
// Assert exactly 80 skills
if (SKILLS_80_V2.length !== 80) {
  throw new Error(`Expected 80 skills, got ${SKILLS_80_V2.length}`);
}
...
for (let r = 1; r <= 8; r++) {
  if (rowCounts[r] !== 10) {
    throw new Error(`Row ${r} has ${rowCounts[r]} items, expected 10`);
  }
}

console.log('✅ Validation passed: exactly 8 rows × 10 items = 80 skills verified!');
